...
def main():
    ...
    summary_df = spark.sql(f"""SELECT cast(date_format(datetaken, "yyyyMM") as int) as datetaken, date_format(datetaken, "yyyy-MM") as ddmm,flatprice, terracedprice, semidetachedprice, detachedprice FROM {fullyQualifiedTableName}
    ...
    n = len(p_dfm.columns)
    for i in range(n):
      if (p_dfm.columns[i] != 'datetaken') & (p_dfm.columns[i] != 'ddmm') :   # yyyyMM is x axis in integer
         ...
         plt.plot(p_dfm['datetaken'],p_dfm[vcolumn], 'bo', label = 'data')  # the actual data
